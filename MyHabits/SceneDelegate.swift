...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        tabBarController.viewControllers = [habitsNavigationController, infoViewController]
        tabBarController.viewControllers?[0].tabBarItem.title = "Привычки"
        tabBarController.viewControllers?[1].tabBarItem.title = "Информация"
        
        tabBarController.viewControllers?.enumerated().forEach {
            $1.tabBarItem.image = $0 == 0 ? UIImage(named: "habits")
            : UIImage(named: "info")
        }
